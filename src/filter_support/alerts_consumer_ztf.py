...
from filter_support.superphot_boom_ztf import run_superphot, post_to_fritz_with_replace
        ...
        while True:
            ...
            if passes_filter:
                ...
                logger.info(
                    ...
                    ztf_id, tracker.total_alerts, tracker.alert_count,
                    tracker.run_count, MAX_RUNS_PER_NIGHT,
                )

                if should_run_superphot(tracker):
                    logger.info("[%s] Triggering superphot run #%d", ztf_id, tracker.run_count + 1)
                    try:
