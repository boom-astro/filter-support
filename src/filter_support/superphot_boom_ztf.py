...
def post_to_fritz_with_replace(
    ...
    if previous_comment_id is not None:
        delete_url = f"{base_url}/sources/{ztf_id}/comments/{previous_comment_id}"
        try:
            del_resp = requests.delete(delete_url, headers=headers)
            del_resp.raise_for_status()
            logger.info("[%s] Deleted previous Fritz comment %s", ztf_id, previous_comment_id)
        except requests.HTTPError as e:
            logger.warning("[%s] Could not delete comment %s: %s", ztf_id, previous_comment_id, e)
...
def run_superphot(ztf_id):
    ...
    if cand_info is None:
        logger.warning("No data found for %s", ztf_id)
        return
    ...
    if (len(df_final.loc[df_final["filter"] == "r"]) <= 2 or
        ...
        logger.warning("Not enough points for %s", ztf_id)
        return
    ...
    if phot.detections.empty or phot.detections['flux'].dropna().empty:
        logger.warning("No valid detections after truncation for %s", ztf_id)
        return
    ...
    for filt in phot._unique_filters:
        ...
        if filt_data.empty:
            logger.warning("Filter %s has no data after normalization for %s", filt, ztf_id)
            return
    ...
    try:
        ...
        logger.exception("SVI sampler failed for %s", ztf_id)
        return None, None
    ...
    if valid_fits.empty:
        logger.warning("Empty fits for %s", ztf_id)
        return None
...
def run_superphot_from_csv(csv_path):
    ...
    if raw.empty:
        logger.warning("[%s] No valid photometry rows in CSV", ztf_id)
        return None
    ...
    if (len(df_final.loc[df_final["filter"] == "r"]) <= 2 or
        len(df_final.loc[df_final["filter"] == "g"]) <= 2):
        logger.warning("[%s] Not enough points", ztf_id)
        return None
    ...
    if phot.detections.empty or phot.detections["flux"].dropna().empty:
        logger.warning("[%s] No valid detections after truncation", ztf_id)
        return None
    ...
    for filt in phot._unique_filters:
        filt_data = phot.detections[phot.detections["filter"] == filt]
        if filt_data.empty:
            logger.warning("[%s] Filter %s empty after normalization", ztf_id, filt)
            return None
    ...
    try:
        ...
        logger.exception("[%s] SVI sampler failed", ztf_id)
        return None
    ...
    if valid_fits.empty:
        logger.warning("[%s] Empty fits", ztf_id)
        return None
    ...
    try:
        ...
        logger.warning("[%s] No valid returns", ztf_id)
        return None
...
def run_batch_from_csv(csv_dir="data/photometry", output_dir="superphot_results"):
    ...
    - A combined results CSV at <output_dir>/superphot_results_ztf_batch.csv
    ...
    results_csv = output_dir / "superphot_results_ztf_batch.csv"
